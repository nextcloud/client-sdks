...
/// struct for typed successes of method [`core_auto_complete_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CoreAutoCompleteGetSuccess {
    Status200(crate::models::CoreAutoCompleteGet200Response),
...
pub async fn core_auto_complete_get(configuration: &configuration::Configuration, search: &str, ocs_api_request: &str, item_type: Option<&str>, item_id: Option<&str>, sorter: Option<&str>, share_types: Option<&str>, limit: Option<i64>) -> Result<ResponseContent<CoreAutoCompleteGetSuccess>, Error<CoreAutoCompleteGetError>> {
    ...
    if let Some(ref local_var_str) = share_types {
        local_var_req_builder = local_var_req_builder.query(&[("shareTypes", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        let local_var_entity: Option<CoreAutoCompleteGetSuccess> = serde_json::from_str(&local_var_content).ok();
